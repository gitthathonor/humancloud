...
<div class=" col-md-3 grid-margin stretch-card">
 ...
 -->
			<div class="py-3 px-3 text-center d-flex justify-content-around">
				<c:choose>
					<c:when test="${!empty sessionScope.principal.userId && empty sessionScope.companyPrincipal.companyId}">
						<div class="btn btn-outline-primary btn-fw">
							<a class="text-black" href="/update/${sessionScope.principal.userId}">회원정보수정</a>
						</div>
						<div class="btn btn-outline-primary btn-fw" onclick='deleteUser("${sessionScope.principal.userId}")'>계정탈퇴</div>
					</c:when>
					<c:when test="${!empty sessionScope.companyPrincipal.companyId && empty sessionScope.principal.userId}">
						<div class="btn btn-outline-primary btn-fw">
					<a class="text-black" href="/company/updateForm/${sessionScope.companyPrincipal.companyId}">기업정보수정</a>
				</div>
				<div class="btn btn-outline-primary btn-fw" onclick='deleteCompany("${sessionScope.companyPrincipal.companyId}")'>기업 삭제</div>
					</c:when>
				</c:choose>
			</div>
		</div>
