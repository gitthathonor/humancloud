<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    <%@ include file="../../layout/header.jsp" %>
        ...
        <script>
            ...
            function makeList(x) {
                let item = `<div id="resumeCard">`;
                for (let list of x) {
                    item += `<div class="card px-4 m-3">`;
                    item += `<div class="card-body row"><div class="col-2"><img src="/img/` + list.resumePhoto + `" class="img-thumbnail" style="width:200px; height:150px;"/></div>`;
                    item += `<div class="col-8 px-5">`
                    item += `<p class="mb-4">` + list.resumeUserId + `</p>`;
                    item += `<p class="fs-30 mb-2 text-black">` + list.resumeTitle + `</p>`;
                    item += `<p class="fs-10 mb-2">` + list.resumeCreatedAt + `</p>`;
                    ...
                    item += `</div></div></div>`;
                    item += '<div style="margin: 0 0 15px 0;"></div>';
                }
                return item;
            }
        </script>
